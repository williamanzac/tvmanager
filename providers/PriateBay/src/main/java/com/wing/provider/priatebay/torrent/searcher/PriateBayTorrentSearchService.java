...
public class PriateBayTorrentSearchService implements TorrentSearchService {
	...
	public List<Torrent> searchTorrent(String showName, int season, int episode) throws Exception {
		String url = MessageFormat.format(searchPattern, baseURL(), searchURL(showName, season, episode));
		final InputStream response = getResponse(url);
		final Document document = htmlReader.read(response);
		return parseXML(document);
	}

	private List<Torrent> parseXML(Document document) {
		System.out.println(document.asXML());
		return null;
	}

	private String searchURL(String show, final int season, final int episode) throws Exception {
